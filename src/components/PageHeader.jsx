import React from 'react'; // Penambahan utama saya
import { LuFilter } from "react-icons/lu";

export default function PageHeader(props) {
    // Fungsi renderBreadcrumb menggunakan React.Fragment, 
    // jadi butuh import React di atas.
    const renderBreadcrumb = () => {
        if (Array.isArray(props.breadcrumb)) {
            return props.breadcrumb.map((item, index) => (
                <React.Fragment key={index}>
                    <span className={`text-gray-400 ${index === props.breadcrumb.length - 1 ? 'font-bold text-hijau' : ''}`}>
                        {item}
                    </span>
        ...
        return <span className="text-gray-400 font-bold text-hijau">{props.breadcrumb}</span>;
    };
